...
class MyBST
{
  private:
    ...
    void balance(BinaryNode*& t)
    {
        ...
        if(height(t->left) > 1 + height(t->right))
            if(height(t->left->left) > height(t->left->right))
                ...
                doubleRotateLeft(t);
        else if(height(t->right) > 1 + height(t->left))
    ...
    void rotateRight(BinaryNode*& t)
    {
        ...
        k->height = std::max(height(k->right), t->height) + 1;
        t = k;
        // code ends
    }
  ...
  public:
    ...
    bool lowestCommonAncestor(const ComparableType& x, const ComparableType& y, ComparableType& lca)
    {
        ...
        BinaryNode* t = root;
        while(true){
            if(x > t->element && y > t->element) t = t->right;
            else if(x < t->element && y < t->element) t = t->left;
            else{
                lca = t->element;
                return true;
            }
